...
function refactorHTML(html) {
    ...
    // 6. Remove footer (will be generated by JS)
    refactored = refactored.replace(
        /<!-- Footer -->[\s\S]*?<\/footer>\n/,
        '<!-- Footer will be generated by common-components.js -->\n'
    );

    // 7. Remove share buttons (will be generated by JS)
    refactored = refactored.replace(
        /<!-- Share Buttons -->[\s\S]*?<\/div>\n[\s\S]*?<\/div>\n/,
        '<!-- Share buttons will be generated by common-components.js -->\n'
    );

    // 8. Remove mobile TOC modal (will be generated by JS)
    refactored = refactored.replace(
        /<!-- Floating TOC Button \(Mobile\) -->[\s\S]*?<!-- Mobile TOC Modal -->[\s\S]*?<\/div>\n[\s\S]*?<\/div>\n/,
        '<!-- Mobile TOC will be generated by common-components.js -->\n'
    );

    // 9. Update scripts section - remove duplicates and add new ones
    // First, remove existing scripts
    refactored = refactored.replace(
        /<!-- Scripts -->[\s\S]*?(<\/body>)/,
        `<!-- Scripts -->
